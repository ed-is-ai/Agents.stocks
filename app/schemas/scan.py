"""Scan, scoring, and analysis schemas."""
...
class StockScan(BaseModel):
    ...
    high_52w: float
    low_52w: float
    high_base: float | None = (
        None  # Highest daily high over the past 10 weeks (pivot entry reference)
    )
    ...
    inst_ownership_pct: float | None = (
        None  # Institutional ownership fraction (0.0–1.0)
    )
    ...
    funds_buying: int | None = (
        None  # WhalWisdom: # filers that increased position last quarter
    )
    funds_selling: int | None = (
        None  # WhalWisdom: # filers that decreased position last quarter
    )
    funds_net: int | None = None  # WhalWisdom: funds_buying minus funds_selling
    congress_buys: int | None = (
        None  # QuiverQuant: congressional buy transactions last 12 months
    )
    congress_sells: int | None = (
        None  # QuiverQuant: congressional sell transactions last 12 months
    )
    senate_buys: int | None = (
        None  # QuiverQuant: Senate-only buy transactions last 12 months
    )
    senate_sells: int | None = (
        None  # QuiverQuant: Senate-only sell transactions last 12 months
    )
    # Market context
    rel_strength_vs_spy: float | None = (
        None  # Stock 52w return minus SPY 52w return (pct pts)
    )
...
class MomentumScore(BaseModel):
    ...
    I: int = Field(ge=0, le=2)  # noqa: E741  # Weinstein stage / SMA structure (institutional proxy)
...
class CANSLIMScore(BaseModel):
    ...
    I: int = Field(ge=0, le=2)  # noqa: E741  # Institutional sponsorship — % held by institutions
...
class StockAnalysis(BaseModel):
    ...
    stage: str
    entry_zone: str = (
        "far"  # "broken_out"|"approaching"|"getting_close"|"extended"|"far"
    )
    ...
    multiyear_breakout: bool = (
        False  # price 0-8% above a significant multi-year base pivot
    )
    multiyear_pivot: float | None = None  # the historical pivot level being broken
    sepa_template: dict[str, bool] | None = (
        None  # 8 Minervini trend template conditions
    )
