...
class TestFallbackLogic:
    ...
    def test_quote_stable_success(self):
        ...
        client.session.get = fake_get  # type: ignore[bad-assignment]
    ...
    def test_quote_both_fail(self):
        ...
        client.session.get = MagicMock(return_value=resp_403)  # type: ignore[bad-assignment]
        result = client.get_quote("^GSPC")
...
class TestResponseNormalization:
    ...
    def test_historical_stable_batch_no_match_falls_back_to_v3(self):
        ...
        client = _make_client()
        batch_data = {
            "historicalStockList": [{"symbol": "SPY", "historical": [{"close": 500}]}]
        }
        ...
        with patch.dict(
            ...
            with (
                ...
                try:
                    ...
                    pytest.fail(
                        "screen_canslim.main() should not exit when historical prices fail"
                    )

            captured = capsys.readouterr()
            assert (
                "EMA fallback" in captured.out
                or "historical data unavailable" in captured.out
            )
