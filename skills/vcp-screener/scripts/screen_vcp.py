...
def fetch_quotes_yfinance(symbols: list[str]) -> dict[str, dict]:
    """Batch-fetch quote data for pre-filter using yfinance (free, no API key).

    Downloads 1 year of daily OHLCV in a single call and derives the fields
    needed by pre_filter_stock: price, yearHigh, yearLow, avgVolume.
    """
    print("    (using yfinance for quotes)", flush=True)
    tickers_str = " ".join(symbols)
    data = yf.download(tickers_str, period="1y", progress=False, auto_adjust=True)
    if data.empty:
        return {}
...
def main():
    ...
    # Batch fetch quotes via yfinance (free, no FMP quota needed)
    print("  Fetching quotes...", end=" ", flush=True)
    all_quotes = fetch_quotes_yfinance(symbols)
